...
class Self_Attn(chainer.Chain):
    ...
    def __call__(self,x):
        ...
        proj_query  = F.transpose(F.reshape(self.query_conv(x), (m_batchsize, -1, width*height)), (0,2,1))
        proj_key =  F.reshape(self.key_conv(x), (m_batchsize, -1, width*height))
        energy =  F.matmul(proj_query, proj_key)
        attention = F.softmax(energy, axis=2) 
        proj_value = F.reshape(self.value_conv(x), (m_batchsize, -1, width*height))
